...
export default function ChatWindow({ wa_id, name, onBack }) {
  ...
  useEffect(() => {
    axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/api/messages/${wa_id}`)
  ...
  const handleSend = async () => {
    ...
    await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/api/messages/send`, {
      wa_id,
      name: "You",
      text: input
    });
  ...
  return (
    <div className="flex flex-col w-full h-screen">
      ...
      <div className="bg-green-600 text-white p-4 flex items-center">
        {onBack && (
          <button className="btn btn-sm btn-ghost text-white mr-2" onClick={onBack}>
            ←
          </button>
        )}
